...
function selectiveMap(array, selector, mapper) {
    ...
    for (let i = 0; i < array.length; i++) { // loops through the array
        let el = array[i]; // set array[i] to el

        if (selector(el, i, array)) { // check if selector is true, then go in
            result.push(mapper(el,i,array)) // the ele that is true gets cb to mapper and return the result, then push
        } else {
            result.push(el); // if the selector is false, push el
        }
    }
