...
function myMap(array, cb) { // you are making changes so you will have to return at the end with new arr
    let newArray= [];

    for (let i = 0; i < array.length; i++) {
        let ele = array[i];
        let res = cb(ele, i, array) // cb make changes and store the changes to res
        newArray.push(res) // push the result into the newArray
    }
    return newArray;
}
